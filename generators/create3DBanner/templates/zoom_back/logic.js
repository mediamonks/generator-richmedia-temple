...
function initBanner() {
  ...
  window.swipeIconTimeline = getSwipeIconTimeline(["#swipeIconSliderDot", "#swipeIconSliderDotRing"], {
      paused: true,
      repeat: 2,
      yoyo: true
    },
...
*/
  ...
  if (e && e.data === "starttimer:TotalEngagement") {                                                                   // add element to hide/show during interaction
  ...
  if (e && e.data === "stoptimer:TotalEngagement") {                                                                    // add element to show/hide after the end of interaction
...
function gotoEndFrame() {                                                                                               // add tl to play/pause in end frame
  window.pulseTL.pause(0);
...
function resetPositions() {                                                                                             // add element to animate when back to position
  window.pulseTL.play();
...
function handleCollapseStart(e) {                                                                                       // add tl to play/pause while collapsing
...
function handleCtaClick() {                                                                                             // update exit link
  clearTimeout(window.endframeTimeout)
  clearTimeout(window.resetPositionTimeout)
...
function handleHostpageScroll(e) {
  if (!window.isExpanded && !window.engagement) {
    ...
    var frameAnimation = Math.max(
        startFrameAnimation,
        Math.min(endFrameAnimation, window.per),
    );
